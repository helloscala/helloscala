...
class SingleValueSessionSerializer[T, V](toValue: T => V, fromValue: V => Try[T])(
  ...
  override def deserialize(r: String) =
    valueSerializer.deserialize(r).flatMap(fromValue)
}
...
object SessionSerializer {
  ...
  implicit def intToStringSessionSerializer: SessionSerializer[Int, String] =
  ...
  implicit def longToStringSessionSerializer: SessionSerializer[Long, String] =
  ...
  private[session] def urlEncode(s: String): String =
    URLEncoder.encode(s, "UTF-8")
  private[session] def urlDecode(s: String): String =
