...
class JdbcUtilsTest extends HelloscalaSpec {
  ...
  "JdbcUtils" should {
    ...
    "toCountSql" in {
      JdbcUtils.toCountSql("SELECT * FROM test WHERE a = ? ORDER BY a asc") mustBe "SELECT count(*) FROM test WHERE a = ?"

      JdbcUtils.toCountSql("SELECT * FROM test WHERE a = ? LIMIT 10") mustBe "SELECT count(*) FROM test WHERE a = ?"

      JdbcUtils.toCountSql("SELECT * FROM test WHERE a = ? offset 10") mustBe "SELECT count(*) FROM test WHERE a = ?"

      JdbcUtils.toCountSql("SELECT * FROM test WHERE a = ? group by a") mustBe "SELECT count(*) FROM test WHERE a = ?"
    }
    ...
    "generateWhere" in {
      ...
      JdbcUtils.generateWhere(Seq("a = ?", "b = ?", Some("c = ?"), None, Some("d = ?"))) mustBe " WHERE a = ? AND b = ? AND c = ? AND d = ? "
    }
