...
class DefaultESClient(configuration: Configuration, appLifecycle: AppLifecycle) extends ESClient with StrictLogging {
  val pathPrefix = "helloscala.persistence.elasticsearch"

  val client: TcpClient =
    ElasticsearchHelper.tcpClient(configuration.get[Configuration](pathPrefix))
